...
uint32_t Imm_S(uint32_t instr){
	return ((uint32_t)(instr & 0xfe000000)) >> 20 
		| ((uint32_t) instr & 0x1f) >> 7;
}
...
int32_t Imm_J(uint32_t instr){
	return ((uint32_t)(instr & 0x80000000) >> 11) 
		| ((instr & 0x7fe00000) >> 20)
		| ((instr & 0x00100000) >> 9)
		| (instr & 0x000ff000);
}
...
void SRAI_exe(CPU *cpu, uint32_t instr){
	cpu->regs[rd(instr)] = cpu->regs[rs1(instr)] >> (int32_t)shamt(instr);
}
...
void SRA_exe(CPU *cpu, uint32_t instr){
	cpu->regs[rd(instr)] = (int32_t)cpu->regs[rs1(instr)] >> (int32_t)cpu->regs[rs2(instr)];
}
...
uint32_t cpu_execute(CPU *cpu, uint32_t instr){
	...
	int func7 = (instr >> 25) & 0x7f;z

	cpu->regs[0] = 0;


	switch(opcode) {
        case OP_LUI:   LUI_exe(cpu, instr); break;
        case OP_AUIPC: AUIPC_exe(cpu, instr); break;
        case OP_JAL:   JAL_exe(cpu, instr); break;
        case OP_JALR:  JALR_exe(cpu, instr); break;

        case OP_BRANCH:
            switch(func3) {
                case F3_BEQ:   BEQ_exe(cpu, instr); break;
                case F3_BNE:   BNE_exe(cpu, instr); break;
                case F3_BLT:   BLT_exe(cpu, instr); break;
                case F3_BGE:   BGE_exe(cpu, instr); break;
                case F3_BLTU:  BLTU_exe(cpu, instr); break;
                case F3_BGEU:  BGEU_exe(cpu, instr); break;
            } break;

        case OP_LOAD:
            switch(func3) {
                case F3_LB:   LB_exe(cpu, instr); break;
                case F3_LH:   LH_exe(cpu, instr); break;
                case F3_LW:   LW_exe(cpu, instr); break;
                case F3_LBU:  LBU_exe(cpu, instr); break;
                case F3_LHU:  LHU_exe(cpu, instr); break;
            } break;

        case OP_STORE:
            switch(func3) {
                case F3_SB:  SB_exe(cpu, instr); break;
                case F3_SH:  SH_exe(cpu, instr); break;
                case F3_SW:  SW_exe(cpu, instr); break;
            } break;

        case OP_OP_IMM:  // Immediate arithmetic
            switch(func3) {
                case F3_ADDI:  ADDI_exe(cpu, instr); break;
                case F3_SLTI:  SLTI_exe(cpu, instr); break;
                case F3_SLTIU: SLTIU_exe(cpu, instr); break;
                case F3_XORI:  XORI_exe(cpu, instr); break;
                case F3_ORI:   ORI_exe(cpu, instr); break;
                case F3_ANDI:  ANDI_exe(cpu, instr); break;
                case F3_SLLI:  SLLI_exe(cpu, instr); break;
                case F3_SRLI_SRAI:
                    switch(func7) {
                        case F7_SRLI: SRLI_exe(cpu, instr); break;
                        case F7_SRAI: SRAI_exe(cpu, instr); break;
                    } break;
            } break;
        ...
        case OP_OP:  // Register-register operations
            switch(func3) {
                case F3_ADD_SUB:
                    switch(func7) {
                        case F7_ADD: ADD_exe(cpu, instr); break;
                        case F7_SUB: SUB_exe(cpu, instr); break;
                    } break;
                case F3_SLL:  SLL_exe(cpu, instr); break;
                case F3_SLT:  SLT_exe(cpu, instr); break;
                case F3_SLTU: SLTU_exe(cpu, instr); break;
                case F3_XOR:  XOR_exe(cpu, instr); break;
                case F3_SRL_SRA:
                    switch(func7) {
                        case F7_SRL: SRL_exe(cpu, instr); break;
                        case F7_SRA: SRA_exe(cpu, instr); break;
                    } break;
                case F3_OR:   OR_exe(cpu, instr); break;
                case F3_AND:  AND_exe(cpu, instr); break;
            } break;
        ...
        case OP_SYSTEM:
            switch(func3) {
                case F3_PRIV:
                    ...
                    switch(funct12) {
                        case F12_ECALL:  ECALL_exe(cpu, instr); break;
                        case F12_EBREAK: EBREAK_exe(cpu, instr); break;
                        case F12_WFI:    WFI_exe(cpu, instr); break;
                    } break;
                case F3_CSRRW:  CSRRW_exe(cpu, instr); break;
                case F3_CSRRS:  CSRRS_exe(cpu, instr); break;
                case F3_CSRRC:  CSRRC_exe(cpu, instr); break;
                case F3_CSRRWI: CSRRWI_exe(cpu, instr); break;
                case F3_CSRRSI: CSRRSI_exe(cpu, instr); break;
                case F3_CSRRCI: CSRRCI_exe(cpu, instr); break;
            } break;
